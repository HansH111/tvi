...
struct editorConfig {
  ...
  int tab_stop;             // Tab width (default: 4)
  int max_undo;             // Undo stack limit (default: 1000)
  int autoindent;           // Auto-indent in insert mode (default: 1)
  int ignore_case;          // Case-insensitive search (default: 0)
  int wrap_search;          // Wrap around when searching (default: 1)
  int highlight_search;     // can come from syntax or config
};
...
int  editorReadFile(const char* filename, char sep,
...
char *editorGetSetting(char *key);
void editorShowSetting(char *key);
void editorShowAllSettings();
void editorProcessSetCommand(char *cmd);
void editorListConfigNames();
