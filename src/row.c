...
int editorRowRxToCx(erow *row, int rx) {
  ...
  while (cx < row->size) {
    if (row->chars[cx] == '\t')
      runtime_rx += (E.tab_stop - 1) - (runtime_rx % E.tab_stop);
    runtime_rx++;
...
void editorUpdateRow(erow *row) {
  ...
  unsigned long long allocsize =
      (unsigned long long) row->size + tabs*(E.tab_stop - 1) + nonprint*9 + 1;
  if (allocsize > UINT32_MAX) {
      printf("Some line of the edited file is too long for tvi\n");
      exit(1);
  }

  row->render = xmalloc(row->size + tabs*(E.tab_stop - 1) + 1);
